...
let rec write_expr = function
    ...
    | S_Index(str, i, tp) -> str ^ ".get(" ^ string_of_int i ^ ")"
...
(* convert marmalade SAST statments into java statements *)
...
and write_stmt d vg = match d with
    ...
    | _ -> raise(Failure("Error: Not a valid statement."))
